...
export const getIdeas = (projectId: string): Idea[] => {
  ...
  const mockIdeas = MOCK_EXISTING_IDEAS.filter(
  ...
  return [...mockIdeas, ...projectIdeas].sort(
...
export const saveIdea = (idea: Idea) => {
  ...
  try {
    ...
    // localStorage quota exceeded - clear old ideas and try again with just the new one
    console.warn(
      "localStorage quota exceeded, clearing old generated ideas...",
      e
    );
    try {
      // Keep only the newest few ideas to avoid quota issues
      const recentIdeas = ideas.slice(-3);
      localStorage.setItem(STORAGE_KEYS.IDEAS, JSON.stringify(recentIdeas));
    } catch (e2) {
      // If still failing, just store idea without image data
      console.warn("Still failing, storing idea reference only...");
      const ideaWithoutImage = { ...idea, imageUrl: undefined };
      localStorage.setItem(
        STORAGE_KEYS.IDEAS,
        JSON.stringify([ideaWithoutImage])
      );
    }
  }
};
